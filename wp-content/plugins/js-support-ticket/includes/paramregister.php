...
function jsst_rewrite_rules( $wp_rewrite ) {
      ...
      $rules['(st-staff-add-ticket|st-announcements|st-staff-feedbacks|st-visitor-message|st-role-permission|st-downloads|st-faqs|st-faq|st-add-department|st-announcement|st-add-announcement|st-add-download|st-add-faq|st-add-article|st-add-category|st-kb-articles|st-kb-article|'.$add_message.'|'.$message.'|'.$message_inbox.'|'.$message_outbox.'|'.$userregister.'|st-add-role|st-add-staff|st-staff-permissions|st-my-tickets|st-staff-my-tickets|st-add-ticket|'.$login.'|st-ticket-status|st-staff-announcements|st-departments|st-staff-downloads|st-staff-faqs|st-control-panel|st-staff-kb-articles|st-staff-categories|st-knowledgebase|st-roles|'.$my_profile.'|st-staffs|st-staff-reports|st-department-reports|st-feed-back|st-print-ticket|st-staff-report|st-ticket|st-agent-help-topics|st-agent-canned-responses|st-add-help-topic|st-gdpr-data-compliance-actions|st-add-canned-response)(/[^/]*)?(/[^/]*)?(/[^/]*)?/?$'] = 'index.php?page_id='.$pageid.'&jsstlayout=$matches[1]&jst1=$matches[2]&jst2=$matches[3]&jst3=$matches[4]';
            ...
            case 'add-ticket':
                ...
                if(!empty($q->query_vars['jst1'])){
                    if(strstr($q->query_vars['jst1'], '_13')){
                        jssupportticket::$_data['sanitized_args']['paidsupportid'] = preg_replace('/\/|_13/', '',$q->query_vars['jst1']);
                    }else{
                        jssupportticket::$_data['sanitized_args']['jssupportticketid'] = str_replace('/', '',$q->query_vars['jst1']);
                    }
                }
            ...
            case 'staff-my-tickets':
                ...
                if(!empty($q->query_vars['jst1'])){
                    if(strstr($q->query_vars['jst1'], '_12')){
                        jssupportticket::$_data['sanitized_args']['uid'] = preg_replace('/\/|_12/', '',$q->query_vars['jst1']);
                    }else{
                        jssupportticket::$_data['sanitized_args']['jssupportticketid'] = str_replace('/', '',$q->query_vars['jst1']);
                    }
                }
